(function () {
  ...
  // network — smaller upload, faster on slow connections, less data used.
  // The server compresses again on arrival (Sharp) as the authoritative
  // pass, so if this fails or the browser doesn't support canvas, the
  // original file is just sent as-is and the server handles it instead.
  function compressImageInBrowser(file, { maxDimension, quality, square }) {
  ...
  async function uploadAvatar(file) {
    ...
    if (file.type && file.type.startsWith('image/')) {
      ...
      try {
        ...
        // Couldn't compress in-browser — send the original, the server
        // compresses on arrival too.
        console.warn('Client-side compression skipped:', err.message);
      }
    }
